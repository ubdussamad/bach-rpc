...
class data(object):
        ...
        def update(self,data):
                '''data_obj.append(self, [username,new_password, authentication, user_id]'''
                assert len(data) == 4
                f_obj = open(self.file,'r')
                file = f_obj.readlines()
                f_obj.close()
                f_obj = open(self.file,'w')
                index = int(data[3])-101 #Because user id starts from 101
...
class administration(object):
        ...
        def _login(self,usr,pwd):
                bach_path = os.path.join( parent(os.path.realpath(__file__),2) ,'C/')
                try:code = check_output( [bach_path+'./bach', '-q' , usr]).decode().strip('\n').split(',')
                except: code  = [*'   ']
                if hexmd5(pwd) == code[0]:
                        print("User id: %s , with auth: %s Logged in."%(code[2],'root' if int(code[1]) else 'general' ))
                        token = hexmd5(time.time())[:7]
                        self.clear_token_cache(code[2]) #Last generated Token of the user will be cleared (If Valid)
                        self.__tokens[token] = [time.time(), int(code[1]),code[2]]
