...
int user_query (char file[],char query[], char *ch) {
  FILE *fp;
  char *line = NULL;
  size_t len = 0;
  ssize_t read;
  ...
  for (short i = 0; (read = getline (&line, &len, fp)) != -1; i++)
    ...
    {
      int end = strlength (line);
      char temp[end - ref_i + 1];
      slice_str (line, &temp, ref_i + 1, end);
      strcpy (ch, temp);
      fclose (fp);
      if (line)
        {
...
// Returns the length of array of characters or string
...
// Appends the target user to
// to the deleted database.
// No entry is deleted while this operation
// this is a simple bypass process
// The entries will be modified periodically and the period will not 
// be co-related to any single or multiple delete
// actions for now.
